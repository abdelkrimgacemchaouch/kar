...
cc----------------------------------------------------------------------
      ...
      real*4 firstRanf
      real*4 depArray(nzmax)

      logical itest(4), jtest(4), ktest, noscat, isign
      real*4 xm(4), xb(4), del(4), lmin, l1, l2, l, newgt
      real*4 xkt4 (nmrmax), efrac (nmrmax,negp1)
      
c.... save arrays
      real*4 xsv(nsvmax), ysv(nsvmax), zsv(nsvmax),
...
c---------- also tmp is in ev, need kev
 ...
 100  continue
      ...
      CALL copyseed ( myIter, mySeed, firstRanf )
...
c-------------------------------------------------- MOVE THE PARTICLE TO THE CORRECT BOUNDARY
 26            idold = id
               id    = id + incr(iside)			  ! zone ID changed according to which side was crossed
